...
void TaskManager::finishTask()
{
    ...
    // FINISHED ยังถือ Task เดิมอยู่จนกว่าจะ clearTask()
    // ดังนั้น BUSY ต้องยังเป็น true
    setBusy(true);
...
bool TaskManager::resumeTask(unsigned long now)
{
    if (!_hasTask)
        return false;
    ...
    {
        Serial.println(
            "[TASK MANAGER] RESUME BLOCKED - DOOR OPEN"
        );

        return false;
    }

    if (_task.getStatus() != TaskStatus::STOPPED)
    {
        return false;
    }

    Serial.println(
        "[TASK MANAGER] RESUME STOPPED TASK"
    );
    ...
    // Task ยังเดิมอยู่ จึงยัง Busy
    setBusy(true);

    Serial.println(
        "[TASK MANAGER] RESUMED"
    );
